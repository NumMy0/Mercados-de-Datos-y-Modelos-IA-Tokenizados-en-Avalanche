...
const handleSetForSale = (price: string) => {
  // Este handler ya no ejecuta la transacción on-chain; el modal realiza la tx y emite el evento.
  // Aquí solo actualizamos el estado local para reflejar el cambio en la UI.
  console.log('setModelForSale (evento recibido):', { modelId: selectedModelId.value, price })

  const idx = userModels.value.findIndex(m => String(m.id) === String(selectedModelId.value))
  if (idx !== -1) {
    userModels.value[idx].forSale = true
    userModels.value[idx].price = `${price} AVAX`
  }
