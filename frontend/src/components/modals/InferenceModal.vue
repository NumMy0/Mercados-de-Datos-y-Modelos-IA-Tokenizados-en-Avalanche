...
const handleNewInference = () => {
  // Limpiar el formulario para una nueva inferencia
  textInput.value = ''
  imageFile.value = null
  imagePreview.value = null
  
  // Limpiar el resultado anterior usando el composable
  ...
  console.log('🔄 Formulario reiniciado para nueva inferencia')
}
    ...
    >
      ...
      >
        ...
        <div class="bg-gray-50 app-dark:bg-gray-800 px-6 py-4 rounded-b-xl flex justify-end gap-3 border-t border-gray-200 app-dark:border-gray-700">
          ...
          <button
            v-if="result"
            @click="handleNewInference"
            :disabled="isInferring"
            class="px-6 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg font-medium hover:from-green-700 hover:to-emerald-700 transition-all shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
