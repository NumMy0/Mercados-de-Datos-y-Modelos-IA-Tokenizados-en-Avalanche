...
class MetadataParser {
    ...
    try {
      ...
      if (typeof content === "string") {
        ...
        if (
          ...
          /[\x00-\x08\x0E-\x1F\x7F-\xFF]/.test(content)
        ) {
          console.log(
            `🔍 isModelFile - Contiene caracteres binarios, es archivo de modelo`
          );
          return true;
        }
        ...
        try {
          ...
          console.log(
            `🔍 isModelFile - JSON válido parseado:`,
            Object.keys(parsed)
          );
          ...
          if (
            !parsed.model_id &&
            !parsed.inference_config &&
            !parsed.model_cid
          ) {
            console.log(
              `🔍 isModelFile - JSON sin estructura de metadatos, probablemente modelo`
            );
            return true;
          }
          ...
          console.log(
            `🔍 isModelFile - Error parseando JSON:`,
            jsonError.message
          );
          console.log(
            `🔍 isModelFile - No es JSON válido, es archivo de modelo`
          );
      ...
      console.log(
        `🔍 isModelFile - Tipo desconocido, asumiendo archivo de modelo`
      );
  ...
  static validate(metadata) {
    if (!metadata || typeof metadata !== "object") {
      throw new Error(
        "Metadatos de IPFS inválidos: el contenido no es un objeto JSON."
      );
    }
    ...
    for (const field of requiredFields) {
      if (!(field in metadata)) {
        throw new Error(
          `Metadatos de IPFS incompletos: falta el campo '${field}'.`
        );
      }
    }
    ...
    for (const field of requiredInferenceFields) {
      if (!(field in config)) {
        throw new Error(
          `Metadatos de IPFS incompletos: falta el campo de configuración de inferencia '${field}'.`
        );
      }
    }
    ...
    const isValidCid =
      metadata.model_cid.startsWith("bafy") ||
    ...
    if (!isValidCid) {
      throw new Error(
        `Metadatos inválidos: model_cid '${metadata.model_cid}' no es un CID válido.`
      );
    }
    ...
    if (metadata.model_hash !== "unknown" && metadata.model_hash.length < 32) {
      throw new Error(
        "Metadatos inválidos: model_hash no válido para verificación."
      );
    }
  ...
  static parse(rawJson) {
    ...
    try {
      ...
      throw new Error(
        "Error de formato JSON: La cadena descargada no es un JSON válido."
      );
    }
